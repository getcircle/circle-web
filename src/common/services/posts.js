...
export function reorderCollections(client, ownerId, ownerType, diffs) {
    const key = getCollectionsForOwnerKey(ownerType, ownerId);
    const request = new services.post.actions.reorder_collections.RequestV1({diffs});
    return new Promise((resolve, reject) => {
        client.send(request)
            .then(response => response.simple(resolve, reject, { result: key, diffs }))
            .catch(error => reject(error));
    });
}
        ...
        initialPromise
            .then((result = {}) => {
                ...
                if (itemsToRemove && itemsToRemove.length) {
                    return removeFromCollection(client, collection, itemsToRemove);
                } else {
...
export function removeFromCollection(client, collection, items) {
    const request = new services.post.actions.remove_from_collection.RequestV1({items});
    return new Promise((resolve, reject) => {
        client.send(request)
            .then(response => response.simple(resolve, reject, {collection, result: collection.id, removedItems: items}))
            .catch(error => reject(error));
    });
}
