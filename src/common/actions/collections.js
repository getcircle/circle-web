...
export function getEditableCollections(profileId) {
    ...
    return {
        [SERVICE_REQUEST]: {
            ...
            bailout: state => {
                const collections = state.get('editableCollections');
                return collections.get('loaded') || collections.get('loading');
            },
        },
    };
}
...
export function reorderCollections(ownerId, ownerType, diffs) {
    return {
        [SERVICE_REQUEST]: {
            types: [
                types.REORDER_COLLECTIONS,
                types.REORDER_COLLECTIONS_SUCCESS,
                types.REORDER_COLLECTIONS_FAILURE,
            ],
            remote: client => requests.reorderCollections(client, ownerId, ownerType, diffs),
        }
    };
}
