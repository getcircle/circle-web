...
const cacheSelector = selectors.createImmutableSelector(
    ...
    (
        ...
        if (postCollectionsState.has(postId)) {
            ...
            if (ids && ids.size) {
                // Only show collections user has access to
                const collectionIds = ids.filter((id) => {
                    return !!editableCollectionIds.get(id);
                });
                collections = retrieveCollections(collectionIds.toJS(), cache);
            }
